...
import { isRedactionCompleted, submitRedaction } from "../../utils/Functions/FunctionsRedaction";
...
const WriteArticle = () => {
  ...
  const [dataToSend, setDataToSend] = useState<DataToSend>(defaultValues);

  const redactionIsCompleted = isRedactionCompleted(defaultValues,dataToSend);
  ...
  const handleSubmit = async() => {
    const [stateSubmit,newArticleData] = await submitRedaction(user!.id_utilisateur!, dataToSend);
    if (stateSubmit){
      showToast("Le jeu a été ajouté avec succès","success","Jeu ajouté");
      navigate(`/article/${newArticleData[0]}/${newArticleData[1]}`)
    } else {
      showToast("Le jeu n'a pas pu être ajouté","error","Erreur ajout jeu");
    
    }
  };

  useEffect(() => {
    fetchCategories().then((data) => setCategories(data.results));
    if (
  ...
  return (
    ...
    <div className="redaction-wrap">
      {dataToSend!["images-data"]["article-cover"].path !== "" && <HeroBanner
        top
        bottom
        left
      ...
      <div className="redaction-actions">
        <div
          className="redaction-actions-item cancel-redaction-article"
          onClick={()=>{
        ...
        <div
          className={`redaction-actions-item validate-redaction-article ${redactionIsCompleted ? "": "disabled"}`}
          onClick={() => {
            if (redactionIsCompleted){
              (handleSubmit())
            } else {
              if (!toast){
                showToast('Tout les champs sont requis',"error","Erreur rédaction !")
              }
      ...
      <section className="article-content">
        <div className="article-content-header">
          <div className="article-content-header-infos infos">
            <label htmlFor="cover-article" className="infos-edit-cover">
              ...
              <p>{dataToSend["images-data"]["article-cover"].path === "" ? "Ajouter une couverture" : "Modifier la couverture"}</p>
              <input
                type="file"
                name="cover-article"
                id="cover-article"
            ...
            <TextareaAutosize
              ...
              placeholder="Donnez un titre à ton article"
              onChange={(event) => {
                updateDataToSend(
                  setDataToSend!,
                  "article-data",
            ...
            <div className="infos-footer">
              <Dropdown
                options={categories}
                value={dataToSend["game-data"].category === "" ? "Catégorie" : dataToSend["game-data"].category}
        ...
        <div className="game-details-wrap">
          <label htmlFor="game-cover" className="game-details-wrap-cover">
            {dataToSend["images-data"]["game-cover"].path !== "" ?
            (<img src={dataToSend["images-data"]["game-cover"].path} alt="" />) : 
            (<div className="game-details-wrap-cover-message"><svg
              xmlns="http://www.w3.org/2000/svg"
              width="52"
              height="52"
              viewBox="0 0 52 52"
        ...
        <div className="article-content-textarea">
          <textarea
            ...
            placeholder="Commence la rédaction de ton article..."
            onChange={(event) => {
              updateDataToSend(
                setDataToSend!,
                "article-data",
